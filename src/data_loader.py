...
class DataLoader:
    ...
    def __init__(self,batch_size:int,data_split:float = 0.95,fast:bool=True) -> None:
        self.fast = fast
        if fast:
            self.env = lmdb.open("IAM/lmdb" ,readonly=True)
        self.data_augmentation = False
        self.curr_idx = 0
        self.batch_size = batch_size
        self.samples = []

        f = open("IAM\gt\words.txt")
        chars = set()
        ...
        for line in f:
            ...
            file_name = f"IAM/img/{file_name_subdir1}/{file_name_subdir2}/{file_base_name}"
